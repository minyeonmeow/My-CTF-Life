...
typedef HMODULE (*oh_LoadLibraryW)(LPCWSTR);
typedef int(__stdcall* msgboxFunc)(HWND, LPCSTR, LPCSTR, UINT);

int main()
{
    wchar_t *kernel32 = L"kernel32.dll";
    char *LoadLibrary = "LoadLibraryW";
    oh_LoadLibraryW toLoadOrNotToLoad = myGetProc(myGetModule(kernel32), LoadLibrary);
    HMODULE user32dll = toLoadOrNotToLoad(L"user32.dll");
    msgboxFunc msgbox = myGetProc(user32dll,"MessageBoxA");
